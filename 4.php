...
$query = "SELECT *
FROM (SELECT year, round(AVG(life_expectancy)) as average_life_expectancy
      FROM ISWARYA.life_expectancy
      group by year
      order by year ASC) c1,
      (SELECT year, 
              round(AVG(life_expectancy_female)) as average_life_expectancy_female
       FROM ISWARYA.female_life_expectancy
       group by year
       order by year asc) c2,
       (SELECT year,
                round(AVG(life_expectancy_male)) as average_life_expectancy_male
        FROM ISWARYA.male_life_expectancy
        group by year
        order by year asc) c3
...
while(($row = oci_fetch_array($s, OCI_BOTH))!= false){
  ...
  //'" < These quotes + Double quotes below on year represent X-Axis > "'
     $chart_data .= "{ year:'".$row["YEAR"]."', average_life_expectancy:".$row["AVERAGE_LIFE_EXPECTANCY"].", average_life_expectancy_female:".$row["AVERAGE_LIFE_EXPECTANCY_FEMALE"].", average_life_expectancy_male:".$row["AVERAGE_LIFE_EXPECTANCY_MALE"]."}, ";
...
<html>
 ...
 <body>
  ...
  <div class="container" style="width:900px;">
   ...
   <h1 align="center">Life Expectancy</h3>   
   <br /><br />
   <div id="chart"></div>
  </div>
</center>
...
Morris.Line({
 ...
 xkey:'year',
 ykeys:['average_life_expectancy','average_life_expectancy_female','average_life_expectancy_male'],
 labels:['Average','Female','Male'],
 hideHover:'auto',
 stacked:false
});
</script>